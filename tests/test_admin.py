...
def test_admin_client(client, db):
    client.post("/auth/register", json={"username": "testadmin", "password": "password"})
...
def test_normal_user(client, db):
    client.post("/auth/register", json={"username": "normaluser", "password": "password"})
...
def test_verify_user(test_admin_client, test_normal_user, db):
    # Verify user
    resp = test_admin_client.post(f"/api/admin/users/{test_normal_user.username}/verify", json={"is_verified": True})
    assert resp.status_code == 200
    ...
    db.close()
    db = SessionLocal()
    user = db.query(User).filter(User.username == test_normal_user.username).first()
    assert user.integration.is_verified is True
    
def test_verify_user_not_found(test_admin_client):
    resp = test_admin_client.post("/api/admin/users/ghost/verify", json={"is_verified": True})
