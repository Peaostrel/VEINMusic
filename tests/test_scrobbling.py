...
def test_auto_achievements_flow(auth_client, db, test_user):
    ...
    user_ach = db.query(UserAchievement).filter_by(
        user_id=test_user.id, achievement_id=ach.id).first()
...
def test_toggle_like(auth_client, db, test_user):
    ...
    db.close()
    db = SessionLocal()
    assert db.query(ScrobbleLike).filter_by(
        user_id=test_user.id, scrobble_id=s.id).count() == 1
    ...
    db = SessionLocal()
    assert db.query(ScrobbleLike).filter_by(
        user_id=test_user.id, scrobble_id=s.id).count() == 0
...
def test_add_comment(auth_client, db, test_user):
    ...
    resp = auth_client.post(
        f"/api/scrobble/{s.id}/comment",
        json={
            "content": "<b>Nice song</b>"})
    assert resp.status_code == 200
...
def test_add_comment(auth_client, db, test_user):
    ...
    db.close()
    db = SessionLocal()
    comment = db.query(ScrobbleComment).filter_by(
...
def test_random_scrobbling_direct(db, test_user):
    ...
    with patch("app.core.redis.get_arq_pool") as mock_arq, \
        ...
        for _ in range(5):
            ...
            progress = random.randint(10, duration)
            source = random.choice(["yandex", "spotify", "lastfm"])
