...
test.describe('Profile Page Flow', () => {
  ...
  test('should load profile successfully and show base elements', async ({ page }) => {
    ...
    await expect(page.locator('h2').filter({ hasText: 'История' })).toBeVisible();
  ...
  test('should load another user profile', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toHaveText('Профиль не найден', { timeout: 10000 });
  });
});
