    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="col-span-1 md:col-span-2 mb-4">
              <label htmlFor="cover-upload" className="block text-sm font-bold text-gray-300 mb-2">Визуальное оформление</label>
              <div className="relative w-full rounded-xl bg-[#282828]/30 border-2 border-dashed border-white/10 hover:border-[var(--accent)] transition-colors group mb-10">
                  <label htmlFor="cover-upload" className="block w-full h-32 md:h-48 cursor-pointer overflow-hidden rounded-xl relative">
                      ...
                      <input id="cover-upload" type="file" accept="image/*" className="hidden" onChange={(e) => onSelectFile(e, 'coverUrl')} />
                  </label>
                  <label htmlFor="avatar-upload" className="absolute -bottom-8 left-6 md:left-10 w-24 h-24 md:w-28 md:h-28 rounded-full bg-[#1e1e1e] border-4 border-[#1e1e1e] cursor-pointer overflow-hidden shadow-2xl group/avatar z-10 hover:border-[var(--accent)]">
                      {data.avatarUrl ? (
                      ...
                      <input id="avatar-upload" type="file" accept="image/*" className="hidden" onChange={(e) => onSelectFile(e, 'avatarUrl')} />
                  </label>
              </div>
          </div>
          <div>
              <label htmlFor="display-name" className="block text-sm font-bold text-gray-300 mb-2">Отображаемое Имя</label>
              <input id="display-name" value={data.displayName} onChange={e=>updateData('displayName', e.target.value)} className="w-full p-3 rounded bg-[#282828]/50 border border-white/10 focus:border-[var(--accent)] text-white focus:outline-none transition-colors" />
          </div>
          <div>
              <label htmlFor="country-select" className="block text-sm font-bold text-gray-300 mb-2">Страна</label>
              <select id="country-select" value={data.country} onChange={e => updateData('country', e.target.value)} className="w-full p-3 rounded bg-[#282828]/50 border border-white/10 focus:border-[var(--accent)] text-white outline-none appearance-none cursor-pointer">
                  <option value="">Выберите страну...</option>
                  {countries.map((c: any) => <option key={c.code} value={c.name}>{c.flag} {c.name}</option>)}
              </select>
          </div>
          <div>
              <label htmlFor="city-input" className="block text-sm font-bold text-gray-300 mb-2">Город</label>
              <div className="relative">
                  <input id="city-input" value={data.city} onChange={e=>updateData('city', e.target.value)} onFocus={() => setIsCityInputFocused(true)} onBlur={() => setTimeout(() => setIsCityInputFocused(false), 200)} placeholder="Введите название..." className="w-full p-3 rounded bg-[#282828]/50 border border-white/10 focus:border-[var(--accent)] text-white focus:outline-none" />
      ...
      <div>
          <label htmlFor="bio-textarea" className="block text-sm font-bold text-gray-300 mb-2">О себе</label>
          <textarea id="bio-textarea" value={data.bio} onChange={e=>updateData('bio', e.target.value)} rows={3} className="w-full p-3 rounded bg-[#282828]/50 border border-white/10 focus:border-[var(--accent)] text-white focus:outline-none resize-none transition-colors"></textarea>
      </div>
