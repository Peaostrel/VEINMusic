            ...
            {THEMES.map(opt => {
                ...
                return (
                    <div 
                        ...
                        className={`p-4 rounded-xl border-2 transition-all flex flex-col gap-3 cursor-pointer ${isLocked ? 'opacity-50 grayscale' : isSelected ? 'border-[var(--accent)] bg-[var(--accent)]/10 shadow-[0_0_15px_var(--accent-glow)]' : 'border-white/10 hover:border-white/30'}`}
                    >
                        <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-4">
                                <div 
                                    ...
                                    style={{
                                        background: opt.isRainbow 
                                            ? 'linear-gradient(45deg, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #8b00ff)'
                                            : opt.isCustom
                                            ? (isThemeCustom ? data.theme : 'linear-gradient(45deg, #ef4444, #3b82f6)')
                                            : opt.color
                                    }}
                                ></div>
                                <div>
                        ...
                        {isSelected && opt.isCustom && (
                            <div className="w-full pt-3 border-t border-white/5 flex items-center gap-3" onClick={e => e.stopPropagation()}>
