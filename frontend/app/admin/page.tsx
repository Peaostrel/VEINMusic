...
function AchievementForm({
  ...
  onSave,
  onCreate,
}: Readonly<AchFormProps>) {
  const patch = (p: Partial<Achievement>) =>
    patchAch(editingAch, newAch, setEditingAch, setNewAch, p);
  const val = <K extends keyof Achievement>(field: K) =>
    getAchField(editingAch, newAch, field);
  ...
  const isTrackOrAlbum = ["specific_track", "specific_album"].includes(
    ruleType,
  );
  ...
  return (
    <div className="space-y-5 relative z-10">
      <div>
        <label
          htmlFor="ach-name"
          className="text-xs text-red-500/70 font-mono mb-1.5 block"
        >
          Название
        </label>
        <input
      ...
      <div className="relative" ref={emojiRef}>
        <span className="text-xs text-red-500/70 font-mono mb-1.5 block">
          Иконка (эмодзи)
        </span>
        <button
          id="ach-icon"
      ...
      <div className="pt-5 border-t border-red-900/30">
        <label
          htmlFor="ach-rule-type"
          className="text-xs text-red-500/70 font-mono mb-1.5 block"
        >
          Тип правила (Авто-выдача)
        </label>
        <select
          id="ach-rule-type"
          value={ruleType}
        ...
        >
          ...
          <option value="specific_album">
            Определенный альбом (X треков с него)
          </option>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                id="ach-rule-value"
                type="number"
                value={val("rule_value") as number}
                onChange={(e) =>
                  patch({ rule_value: Number.parseInt(e.target.value) || 0 })
                }
                className="w-full bg-[#1a1010] border border-red-900/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-red-500 transition shadow-inner"
                placeholder="100"
              />
            ...
            {isSpecific && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 w-full">
                <div>
                  <label
                    htmlFor="ach-rule-target"
                    className="text-xs text-red-500/70 font-mono mb-1.5 block"
                  >
                    Ссылка (URL трека/альбома)
                  </label>
                  <input
                    id="ach-rule-target"
                    type="text"
                    value={val("rule_target") as string}
                    onChange={(e) => patch({ rule_target: e.target.value })}
                    className="w-full bg-[#1a1010] border border-red-900/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-red-500 transition shadow-inner"
                    placeholder="https://music.yandex."
                  />
                </div>
                <div>
                  <label
                    htmlFor="ach-rule-meta"
                    className="text-xs text-red-500/70 font-mono mb-1.5 block"
                  >
                    Слово в описании (для ссылки)
                  </label>
                  <input
                    id="ach-rule-meta"
                    type="text"
                    value={val("rule_meta") as string}
                    onChange={(e) => patch({ rule_meta: e.target.value })}
                    className="w-full bg-[#1a1010] border border-red-900/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-red-500 transition shadow-inner"
                    placeholder="Например: Джизус"
                  />
                </div>
              </div>
            )}
        ...
        <div className="space-y-4 mt-4">
          {isTrackOrAlbum && (
            <div>
              <label
                htmlFor="ach-target-image"
                className="text-xs text-red-500/70 font-mono mb-1.5 block"
              >
                URL картинки (сгенерируется само)
              </label>
              <input
                id="ach-target-image"
                type="text"
                value={val("target_image") as string}
          ...
          <div>
            ...
            <input
              id="ach-reward-xp"
              type="number"
              value={val("reward_xp") as number}
              onChange={(e) =>
                patch({ reward_xp: Number.parseInt(e.target.value) || 0 })
              }
              className="w-full bg-[#1a1010] border border-red-900/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-red-500 transition shadow-inner"
...
export default function AdminPanel() {
  ...
  const loadData = () => {
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/stats`,
  ...
  const handleSaveUserEdit = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${editingUser!.username}`,
        {
          credentials: "include",
          method: "PUT",
  ...
  const handleWipeScrobbles = async (username: string) => {
    if (
      !confirm(
        `ВНИМАНИЕ! Полностью стереть историю треков пользователя ${username}? Это нельзя отменить!`,
      )
    )
      return;
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${username}/scrobbles`,
  ...
  const handleDeleteUser = async (username: string) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${username}`,
        { credentials: "include", method: "DELETE" },
      );
      if (res.ok) {
        loadData();
  ...
  const handleEditLevel = async (username: string, currentLevel: number) => {
    const newLvl = prompt(
      `Новый уровень для ${username}:`,
      String(currentLevel),
    );
    if (
      !newLvl ||
      Number.isNaN(Number.parseInt(newLvl)) ||
      Number.parseInt(newLvl) < 1
    )
      return;
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${username}/level`,
        {
          credentials: "include",
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleToggleVerify = async (
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${username}/verify`,
        {
          credentials: "include",
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleGiveAch = async (username: string) => {
    const achs = stats?.achievements
      .map((a: Achievement) => `${a.id} - ${a.name}`)
      .join("\n");
    const achId = prompt(`ID ачивки для ${username}:\n\n${achs}`);
    if (!achId || Number.isNaN(Number(achId))) return;
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/users/${username}/achievements`,
        {
          credentials: "include",
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleCreateAch = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/achievements`,
        {
          credentials: "include",
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...newAch,
            rule_value: Number.parseInt(String(newAch.rule_value)) || 0,
            reward_xp: Number.parseInt(String(newAch.reward_xp)) || 0,
          }),
        },
      );
      if (res.ok) {
  ...
  const handleUpdateAch = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/achievements/${editingAch.id}`,
        {
          credentials: "include",
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...editingAch,
            rule_value: Number.parseInt(String(editingAch.rule_value)) || 0,
            reward_xp: Number.parseInt(String(editingAch.reward_xp)) || 0,
          }),
        },
      );
      if (res.ok) {
  ...
  const handleDeleteAch = async (id: number) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/achievements/${id}`,
        { credentials: "include", method: "DELETE" },
      );
      if (res.ok) {
        loadData();
  ...
  const handleDeleteTrack = async (id: number) => {
    if (
      !confirm(
        `Точно удалить этот трек из глобальной БД? Все скробблы с ним будут УНИЧТОЖЕНЫ!`,
      )
    )
      return;
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admin/tracks/${id}`,
        { credentials: "include", method: "DELETE" },
      );
      if (res.ok) {
        loadData();
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-white pb-20 pt-20">
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label
                  ...
                  className="text-xs text-red-500/70 font-mono mb-1 block"
                >
                ...
                <input
                  ...
                  className="w-full bg-[#1a1010] border border-red-900/30 rounded px-3 py-2 text-white outline-none focus:border-red-500"
                />
              </div>
              <div>
                <label
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#1a1010] border border-red-900/30 rounded px-3 py-2 text-white outline-none focus:border-red-500 resize-none"
                ></textarea>
              </div>
            </div>
      ...
      <div className="flex items-center gap-4 mb-8 border-b border-red-900/30 pb-4">
        <div className="w-12 h-12 bg-red-500/10 rounded-xl flex items-center justify-center text-red-500 font-bold text-xl shadow-[0_0_15px_rgba(239,68,68,0.2)]">
          V
        </div>
        <div>
          <h1 className="text-2xl font-black text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.3)]">
            VEIN CONTROL ROOM
          </h1>
          <p className="text-red-900/80 text-sm font-mono">
            Доступ уровня: Разработчик
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-br from-[#1a1010] to-[#120a0a] border border-red-900/30 p-6 rounded-2xl shadow-lg relative overflow-hidden group">
          <div className="text-red-500/50 text-xs mb-2 font-mono uppercase tracking-widest relative z-10">
            Всего пользователей
          </div>
          <div className="text-4xl font-black text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.3)] relative z-10">
            {stats.total_users}
          </div>
        </div>
        <div className="bg-gradient-to-br from-[#1a1010] to-[#120a0a] border border-red-900/30 p-6 rounded-2xl shadow-lg relative overflow-hidden group">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#1a1010] border-b border-red-900/30">
                <th className="p-4 font-mono text-xs text-red-500/70 uppercase tracking-widest">
                  Юзер
                </th>
                <th className="p-4 font-mono text-xs text-red-500/70 uppercase tracking-widest">
                  Скробблы
              ...
              {stats.users.map((u) => {
                ...
                return (
                  ...
                  >
                    <td className="p-4">
                      <div className="font-bold flex items-center gap-2">
                        <img
                          ...
                          className="w-8 h-8 rounded bg-black border border-red-900/50"
                          alt="Ava"
                        />
                        <div>
                          <div className="flex items-center gap-1 text-white">
                            {u.username}
                            {u.is_dev && (
                              <span className="bg-red-500/20 text-red-500 px-1 py-0.5 rounded text-[8px] uppercase font-black border border-red-500/30">
                                DEV
                              </span>
                            )}
                          </div>
                    ...
                    <td className="p-4 font-mono text-sm text-emerald-500 font-bold">
                      {level}
                    </td>
                    <td className="p-4 flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded text-xs font-bold transition border ${u.is_verified ? "bg-red-900/10 border-red-900/30 text-red-500 hover:bg-red-900/30" : "bg-cyan-500/10 border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20"}`}
                      >
        ...
        <div className="bg-[#120a0a] border border-red-900/30 rounded-2xl p-6 shadow-xl animate-in fade-in zoom-in-95 duration-300">
          <div className="flex justify-between items-center mb-6">
            ...
            <p className="text-xs text-gray-500">
              Удаление трека здесь навсегда сотрет его из историй всех юзеров.
            </p>
          </div>
          <div className="space-y-2 max-h-[700px] overflow-y-auto custom-scrollbar pr-2">
            {stats.tracks?.map((t) => (
              <div
                key={t.id}
                className="bg-[#1a1010] border border-red-900/10 hover:border-red-500/50 transition-colors p-3 rounded-xl flex items-center justify-between group"
              >
                <div className="flex items-center gap-4 truncate">
                  {t.cover_url ? (
                    <img
                      src={t.cover_url}
                      className="w-12 h-12 rounded object-cover border border-white/5 shrink-0"
                      alt="cover"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded bg-gradient-to-br from-[#282828] to-[#121212] border border-white/5 flex items-center justify-center text-sm text-yellow-500/80 shrink-0 shadow-inner">
                      🎵
                    </div>
                  )}
                  <div className="truncate">
                    ...
                    <div className="text-[9px] text-red-500/50 font-mono mt-0.5">
                      ID: {t.id} | {t.track_url || "No URL"}
                    </div>
                  </div>
                </div>
                <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start animate-in fade-in zoom-in-95 duration-300">
          <div className="bg-[#120a0a] border border-red-900/30 rounded-2xl p-6 shadow-xl relative overflow-hidden">
            <h3 className="font-bold mb-6 text-lg">
              {editingAch ? "Редактировать ачивки" : "Создать новую"}
            </h3>
            ...
            <div className="space-y-4 max-h-[700px] overflow-y-auto custom-scrollbar pr-2">
              {stats.achievements.length === 0 ? (
                ...
                stats.achievements.map((a) => (
                  <div
                    key={a.id}
                    className="bg-[#1a1010] border border-red-900/20 p-4 rounded-xl flex items-center justify-between group hover:border-red-500/50 transition-colors shadow-inner"
                  >
                    <div className="flex items-start gap-4 pr-4">
                      {a.target_image ? (
                        <img
                          src={a.target_image}
                          alt="Ach"
                          className="w-10 h-10 rounded object-cover shadow-[0_0_10px_rgba(255,255,255,0.1)] shrink-0"
                        />
                      ) : (
                        <span className="text-4xl leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.1)] shrink-0">
                          {a.icon}
                        </span>
                      )}
                      <div>
                        <div className="font-bold text-white text-sm flex items-center gap-2">
                          ...
                          {a.rule_type !== "manual" && (
                            <span
                              className="text-[10px] text-green-500 bg-green-500/10 px-1.5 py-0.5 rounded border border-green-500/20"
                        ...
                        <div className="text-[11px] text-gray-400 leading-tight mt-1.5">
                          {a.description}
                        </div>
                      </div>
                    </div>
