        ...
        async with httpx.AsyncClient(headers=headers, timeout=5.0) as client:
            if "music.yandex.ru" in url:
                if TRACK_PATH in url:
                    ...
                    if albums:
                        return albums[0].get("genre")
                elif "/album/" in url:
                    album_id = url.split(
                        '/album/')[1].split('/')[0].split('?')[0]
...
def format_history_item(
    ...
    if counters:
        ...
        data["comments_count"] = counters.get(
            scrobble.id, {}).get("comments", 0)
    elif db:
        data["likes_count"] = db.query(ScrobbleLike).filter_by(
            scrobble_id=scrobble.id).count()
        data["comments_count"] = db.query(ScrobbleComment).filter_by(
            scrobble_id=scrobble.id).count()
    return data
...
def _check_favorite(track: Track, user: User) -> bool:
    fav_art = user.profile.favorite_artist.lower() if (
        user.profile and user.profile.favorite_artist) else ""
    fav_trk = user.profile.favorite_track.lower() if (
        user.profile and user.profile.favorite_track) else ""
    fav_alb = user.profile.favorite_album.lower() if (
        user.profile and user.profile.favorite_album) else ""

    t_artist = track.artist.lower()
    t_title = track.title.lower()
    t_album = track.album.lower() if track.album else ""

    if fav_art and fav_art in t_artist:
        return True
    ...
    if scrobbles_today >= 5:
        ...
        if last_streak_date != today_str:
            if last_streak_date == yesterday_str:
                user.integration.current_streak = (
                    user.integration.current_streak or 0) + 1
            else:
...
def _determine_is_new(db: Session,
    ...
    if not last_scrobble or last_scrobble.track_id != track.id:
        if last_scrobble and last_scrobble.is_playing and (
